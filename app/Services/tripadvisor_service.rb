    ...
    search_queries.each do |search_query|
      ...
      if data["data"]&.any?
        ...
        locations = data["data"].select do |loc|
          ...
          location_matches = (
            address.include?(base_city_name) ||
          ...
          if address.present? && address.length > 5
            ...
            # No address data - accept the location (API search should be accurate)
            true
          end
        end.take(5) # Increased from 3 to 5

        Rails.logger.info "Found #{locations.length} unique locations for photos from #{data['data']&.length || 0} total results"
    ...
    if data["data"]&.any?
      ...
      actual_locations = data["data"].reject do |loc|
        ...
        if address.present? && address.length > 5
          ...
          !(address.include?(base_city_name) || address.include?(destination_country.downcase))
        else
          false # Keep locations without address data
        end
      end

      Rails.logger.info "Filtered out #{data['data'].length - actual_locations.length} tours/activities/wrong-locations"
      ...
      if actual_locations.any?
        ...
        address = actual_locations.first["address_obj"]&.dig("address_string") || "No address"
        Rails.logger.info "Selected location ID: #{location_id} (#{actual_locations.first['name']}) - Address: #{address}"
        return location_id
      else
